...
void node::add_before(node* top,int x,int k){
	...
	if(top!=NULL){
		...
		cout<<"element "<<x<<" is not in this linkeed list\n";
}
...
int main(){
	...
	cout<<"enter x and k\n";
	cin>>x>>k;
	head->add_after(head,x,k);
	cout<<"result after adding "<<k<<" after "<<x<<": ";
	head->print(head);
	head->add_before(head,x,k);
	cout<<"result after adding "<<k<<" before "<<x<<": ";
	head->print(head);
	head->del(head,k);
	cout<<"result after deleting "<<k<<": ";
	head->print(head);
	head->search(head,x);
return 0;
}
